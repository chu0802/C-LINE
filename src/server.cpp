...
unordered_map<string, vector<string>> Friend_Agree;


mutex c_mtx, nt_mtx, np_mtx, fr_mtx, ht_mtx, off_mtx, std_mtx, ps_mtx, fr_ag_mtx, sock_mtx;
...
void client_handle(int fd){
	...
	while(1){
		...
		if(Start_Correct && End_Correct){
			...
			ssize_t  ret_val;
			if(ans != NULL){
				ret_val = send(fd, ans, sizeof(Request), 0);
...
int main(int argc, char **argv){
	...
	ifstream ps("signature.txt");
	if(ps.is_open()){
		string line;
		string user = "";
		while(getline(ps, line)){
			string user = line.substr(0, line.find(":"));
			string user_ps = line.substr(line.find(":") + 1);
			Personal_Signature[user] = (user_ps);
		}
	}

	ps.close();

	ifstream fr_ag("friend_agree.txt");

	if(fr_ag.is_open()){
		string line;
		string user = "";
		while(getline(fr_ag, line)){
			string user = line.substr(0, line.find(":"));
			string user_friend = line.substr(line.find(":") + 1);
			Friend_Agree[user].push_back(user_friend);
		}
	}
	fr_ag.close();
